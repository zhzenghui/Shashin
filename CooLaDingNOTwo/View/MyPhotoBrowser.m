...
#import "MyPhotoBrowserPrivate.h"
...
//    self.navigationController.navigationBarHidden=YES;
    ...
    UIButton *leftButton =  // back按钮
    [[Button share] addTarget:self
                         addT:@selector(tapBack)
                    imagePath:@"分享"
                         rect:CGRectMake(10, 20, 44, 44)
                       toView:self.view];
    ...
    if (self.desplayType==1||self.desplayType==2) {
    //右侧按钮
    self.collectButton =
    [[Button share] addToView:self.view
                    addTarget:self
                         rect:CGRectMake(KDeviceWidth - 54, 0, 44, 44)
                          tag:10000
                       action:@selector(doCollect)];
    self.collectButton.tag = 5000;  // 5000为未收藏，5001为已收藏
    UIBarButtonItem *rightItem =
    [[UIBarButtonItem alloc] initWithCustomView:self.collectButton];
    self.navigationItem.rightBarButtonItem = rightItem;
    
    self.collectImage =
    [[UIImageView alloc] initWithFrame:CGRectMake(10, 10, 24, 24)];
    self.collectImage.image = [UIImage imageNamed:@"collect_empty_ico"];
    [self.collectButton addSubview:self.collectImage];
     }
    ...
    //创建信息说明布局
    self.infoView = [[UIImageView alloc] initWithFrame:CGRectMake(0, KDeviceHeight-40, KDeviceWidth, 40)];
//    self.infoView.translatesAutoresizingMaskIntoConstraints = NO;
    [self.infoView setBackgroundColor:[UIColor colorWithRed:104.0 / 255.0
                                                      green:104.0 / 255.0
                                                       blue:104.0 / 255.0
                                                      alpha:0.6]];
    ...
    //图片名称
    self.imageNameLable = [[UILabel alloc] initWithFrame:CGRectMake(5, 5, 200, 30)];
//    self.imageNameLable.translatesAutoresizingMaskIntoConstraints = NO;
    [self.imageNameLable setText:self.atlasName];
    self.imageNameLable.font = [UIFont fontWithName:@"Helvetica-Bold" size:14];
    [self.imageNameLable setTextColor:[UIColor whiteColor]];
    [self.infoView addSubview:self.imageNameLable];
    
    //图片索引
    self.imageIndexLable = [[UILabel alloc] initWithFrame:CGRectMake(KDeviceWidth-105, 5, 100, 30)];
    self.imageIndexLable.textAlignment = NSTextAlignmentRight;
    self.imageIndexLable.font = [UIFont fontWithName:@"Helvetica-Bold" size:14];
    [self.imageIndexLable setTextColor:[UIColor whiteColor]];
    [self.infoView addSubview:self.imageIndexLable];
    
    //判断信息布局是否显示
    if (self.desplayType == 1||self.desplayType == 3) {
...
- (void)setNavBarAppearance:(BOOL)animated {
    [self.navigationController setNavigationBarHidden:NO animated:animated];//之前的代码
//     [self.navigationController setNavigationBarHidden:YES animated:animated];//永久隐藏
...
- (void)restorePreviousNavBarAppearance:(BOOL)animated {
    if (_didSavePreviousStateOfNavBar) {
        [self.navigationController setNavigationBarHidden:_previousNavBarHidden animated:animated];//之前的代码
//        [self.navigationController setNavigationBarHidden:_previousNavBarHidden animated:animated];//永久隐藏
        UINavigationBar *navBar = self.navigationController.navigationBar;
...
- (void)configurePage:(MWZoomingScrollView *)page forIndex:(NSUInteger)index {
    ...
    [_delegate getCurrentMWPhoto:page.photo photoAtIndex:page.index];
}
...
- (void)updateNavigation {//每次滑动会调用多次
...
//        }
        if(self.desplayType==1){
        MWPhoto *mWPhoto=_photos[_currentPageIndex];
        DLog(@"当前的收藏标识为：%@！！！！！",mWPhoto.isCollect);
        if ([mWPhoto.isCollect isEqual:@"1"]) {
            self.collectImage.image = [UIImage imageNamed:@"collect_full_ico"];
            self.collectButton.tag = 5001;
        } else {
            self.collectImage.image = [UIImage imageNamed:@"collect_empty_ico"];
            self.collectButton.tag = 5000;
        }
        }else if(self.desplayType==2){
            if (self.atlasFlag) {
                self.collectImage.image = [UIImage imageNamed:@"collect_full_ico"];
                self.collectButton.tag = 5001;
            } else {
            ...
            if (_photos != nil && _photos.count > 0) {
                [self.imageIndexLable
                 setText:[NSString stringWithFormat:@"%lu/%lu", (unsigned long)(_currentPageIndex+1),(unsigned long)numberOfPhotos]];
            } else {
                [self.imageIndexLable setText:@"0/0"];
            }
...
#pragma mark collect custom method
//收藏当前图片或图集
- (void)doCollect {
    //修改收藏图标
    if (self.collectButton.tag == 5000) {
        self.collectImage.image = [UIImage imageNamed:@"collect_full_ico"];
        self.collectButton.tag = 5001;
    ...
    NSString *number = nil;
    [param setValue:[Utils getUserTokenID] forKey:@"tokenId"];
    if (self.desplayType == 1) {             //单图
        if (self.collectButton.tag == 5001) {  //未收藏->已收藏
            url = [NSString stringWithFormat:@"%@%@", kDomain, @"collect/userCollectImage"];
            number = @"1";
            
        } else {  //已收藏->未收藏
            url = [NSString  stringWithFormat:@"%@%@", kDomain, @"collect/userCancelCollectImage"];
            number = @"0";
        }
        MWPhoto *mWPhoto=_photos[_currentPageIndex];
        mWPhoto.isCollect= number;
        [param setValue:mWPhoto.imageId forKey:@"sgId"];              //图片编号
    }
    else {
        //套图
        if (self.collectButton.tag == 5001) {  //未收藏->已收藏
            url = [NSString stringWithFormat:@"%@%@", kDomain, @"collect/userCollectAtlas"];
        }
        else {  //已收藏->未收藏
            url = [NSString stringWithFormat:@"%@%@", kDomain, @"collect/userCancelCollectAtlas"];
        }
        [param setValue:self.atlasId forKey:@"atlasId"];    //图集编号
        self.atlasFlag = (self.collectButton.tag == 5001);  //修改套图收藏标识
        
        DLog(@"套图是否被收藏%@！！！！！！！！", self.atlasFlag ? @"YES" : @"NO");
    }
    DLog(@"接口地址%@和参数%@!!!!!", url, param.description);
    //接口调用管理
    ...
    [httpManager POST:url
           parameters:param
              success:^(AFHTTPRequestOperation *operation, id responseObject) {
                  ...
                  //        DLog(@"接口返回数据 :%@", resultDictionary.description);
                  NSInteger resultFlag =
                  [[resultDictionary objectForKey:@"status"] integerValue];
                  //        DLog(@"添加结果%ld", (long)resultFlag);
              ...
              failure:^(AFHTTPRequestOperation *operation, NSError *error) {
                  ...
                  [self.view makeToast:@"网络连接异常，操作失败！"];
                  //        self.collectButton.userInteractionEnabled = YES;
              }];
}
